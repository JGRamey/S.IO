...
from yggdrasil.agents.mcp_agent_manager import MCPAgentManager, MCPAnalysisRequest, AnalysisType
...
class YggdrasilMCPServer:
    """Main MCP Server for Yggdrasil system with AI agent integration"""
    ...
    def _setup_handlers(self):
            ...
            return ListToolsResult(
                tools=[
                    ...
                    Tool(
                        ...
                        description="Perform AI-powered content analysis using various agents",
                        inputSchema={
                            "type": "object",
                            "properties": {
                                "text": {
                                ...
                                "analysis_type": {
                                    "type": "string",
                                    "enum": ["theme_analysis", "doctrine_analysis", "fallacy_detection", 
                                           "translation_analysis", "text_sourcing", "storage_optimization",
                                           "content_analysis", "comprehensive"],
                                ...
                                "domain": {
                                    "type": "string",
                                    "description": "Content domain (optional)"
                                },
                                "tradition": {
                                    "type": "string",
                                    "description": "Religious/philosophical tradition (optional)"
                                },
                                "language": {
                                    "type": "string",
                    ...
                    Tool(
                        name="get_analysis_types",
                        description="Get list of available AI analysis types",
                        inputSchema={
                            "type": "object",
                            "properties": {}
                        }
                    ...
                    Tool(
                        ...
                        description="Get performance statistics for AI agents",
                        inputSchema={
                            "type": "object",
                            "properties": {}
                        }
        ...
        async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> CallToolResult:
            ...
            if not hasattr(self.agent_manager, 'agents') or not self.agent_manager.agents:
            ...
            elif name == "ai_content_analysis":
                return await self._ai_content_analysis(arguments)
            
            elif name == "get_analysis_types":
                return await self._get_analysis_types()
            
            elif name == "agent_performance_report":
                return await self._agent_performance_report()
    ...
    async def _ai_content_analysis(self, arguments: Dict[str, Any]) -> CallToolResult:
        """Perform AI-powered content analysis"""
        
        try:
            # Parse analysis type
            analysis_type_str = arguments["analysis_type"]
            ...
            # Perform analysis
            result = await self.agent_manager.analyze_content(request)
            ...
            return CallToolResult(
                content=[TextContent(
                    type="text",
                    text=f"AI Content Analysis Results:\n{json.dumps(response, indent=2)}"
                )]
            )
            ...
            return CallToolResult(
                content=[TextContent(
                    type="text",
                    text=f"Error in AI content analysis: {str(e)}"
                )]
            )
    
    async def _get_analysis_types(self) -> CallToolResult:
        """Get available analysis types"""
        
        try:
            analysis_types = self.agent_manager.get_available_analyses()
            
            return CallToolResult(
                content=[TextContent(
                    type="text",
                    text=f"Available AI Analysis Types:\n{json.dumps(analysis_types, indent=2)}"
                )]
            )
            ...
            return CallToolResult(
                content=[TextContent(
                    type="text",
                    text=f"Error getting analysis types: {str(e)}"
                )]
            )
    
    async def _agent_performance_report(self) -> CallToolResult:
        """Get agent performance statistics"""
        
        try:
            stats = self.agent_manager.get_performance_stats()
            
            return CallToolResult(
                content=[TextContent(
                    type="text",
                    text=f"Agent Performance Report:\n{json.dumps(stats, indent=2)}"
                )]
            )
            ...
            return CallToolResult(
                content=[TextContent(
                    type="text",
                    text=f"Error getting performance report: {str(e)}"
                )]
            )
