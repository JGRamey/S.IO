#!/usr/bin/env python3
"""
Rename Solomon system to Yggdrasil and extend capabilities
"""
...
def rename_solomon_to_yggdrasil():
    """Rename Solomon components to Yggdrasil branding"""
    
    print("🌳 REBRANDING SOLOMON TO YGGDRASIL")
    print("="*40)
    
    root = Path("/Users/grant/Desktop/Solomon/Database/S.IO")
    
    # Step 1: Create new yggdrasil directory structure
    yggdrasil_dir = root / "yggdrasil"
    yggdrasil_dir.mkdir(exist_ok=True)
    
    # Copy solomon directory to yggdrasil
    solomon_dir = root / "solomon"
    if solomon_dir.exists():
        # Copy contents
        for item in solomon_dir.iterdir():
            if item.is_dir():
                shutil.copytree(item, yggdrasil_dir / item.name, dirs_exist_ok=True)
            else:
                shutil.copy2(item, yggdrasil_dir / item.name)
        print("✅ Copied Solomon structure to Yggdrasil")
    
    # Step 2: Update import statements and references
    update_files = [
        "yggdrasil/database/models.py",
        "yggdrasil/scraping/__init__.py", 
        "yggdrasil/scraping/base_scraper.py",
        "yggdrasil/scraping/scraping_manager.py",
        "yggdrasil/scraping/scraper_factory.py"
    ]
    ...
    for file_path in update_files:
        ...
        if full_path.exists():
            ...
            # Update import paths
            content = content.replace("..database.models", "yggdrasil.database.models")
            content = content.replace("solomon.database", "yggdrasil.database")
            content = content.replace("solomon.scraping", "yggdrasil.scraping")
            content = content.replace("Solomon project", "Yggdrasil Knowledge System")
            full_path.write_text(content)
            print(f"✅ Updated {file_path}")
    
    print("✅ Rebranding complete!")

def extend_schema_for_broader_domains():
    """Extend the schema to handle broader knowledge domains"""
    
    print(f"\n📚 EXTENDING SCHEMA FOR BROADER DOMAINS")
    print("="*45)
    
    # Create enhanced models file
    enhanced_models = '''"""Enhanced Yggdrasil database models for broad knowledge domains."""
...
'''
    ...
    models_file = Path("/Users/grant/Desktop/Solomon/Database/S.IO/yggdrasil/database/enhanced_models.py")
    models_file.write_text(enhanced_models)
    print("✅ Created enhanced models with broader domain support")

if __name__ == "__main__":
    rename_solomon_to_yggdrasil()
    extend_schema_for_broader_domains()
    print(f"\n🎉 YGGDRASIL KNOWLEDGE SYSTEM READY!")
    print("Next: Create new scrapers and book import capabilities")
